#!/usr/bin/env bash
# Build the nautilus-image-converter-gtk4 .deb for Ubuntu 24.04 (noble) and
# 26.04 (resolute) in clean containers, from the committed tree only
# (git archive HEAD).
#
#   packaging/build-debs.sh [noble|resolute|all]      (default: all)
#
# Output: dist/debs/<codename>/. Install the package built for your release:
# the plugin must match that release's Nautilus, and configure bakes in its
# ImageMagick binary (convert on 24.04, magick on 26.04).
set -Eeuo pipefail

NAME=nautilus-image-converter-gtk4
